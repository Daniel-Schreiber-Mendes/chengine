...
void mainMenuState_construct(State *const s)
{
    ...
    checs_command_subscribe(<C:WindowCloseCommand>, window_close_command_callback);
    checs_command_subscribe(<C:FramebufferSizeCommand>, framebuffer_size_command_callback);
    checs_command_subscribe(<C:KeyCommand>, key_command_callback);
    checs_command_subscribe(<C:ScrollCommand>, scroll_command_callback);
    checs_command_subscribe(<C:MouseButtonCommand>, mouseButton_command_callback);
...
static void key_command_callback(KeyCommand *kc)
{
    ...
    else if (kc->key == GLFW_KEY_C && kc->action == GLFW_PRESS)
    {
        for (int i=0; i<20; ++i)
        {
            EntityId e = checs_entity(1 << <Renderable> | 1 << <Velocity> | 1 << <Collidable>);

            checs_tags_add(1 << <T:Gravitatable>, e);


            Transform *t = checs_component_get(<Transform>, e);
            t->pos[0] = i * 0.003f;
            t->pos[1] = i * 0.003f;
            ...
            Collidable *c = checs_component_get(<Collidable>, e);
            c->r = 0.125;
            c->mass = 0.5;
    ...
    {
        EntityId e = checs_entity_get_by_tag(1 << <T:Gravitatable>);
...
static void scroll_command_callback(ScrollCommand *com)
{
    Camera *c;
    EntityId entity;
    for (uintEC i=0; c = checs_components_foreach(<Camera>, &entity, i); ++i)
    {
...
static void mouseButton_command_callback(MouseButtonCommand *com)
{
    if (com->action == GLFW_PRESS)
    {
        ...
        Velocity *v = checs_component_get(<Velocity>, player);

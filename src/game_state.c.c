...
void game_state_construct(State *s)
{
    ...
    {
        EntityId player = checs_entity(1 << 5 | 1<< 1 | 1 << 2 | 1 << 7 | 1 << 4);
        ...
        Renderable *r = checs_component_get(5, player);
        r->scale[0] = 0.2;
        r->scale[1] = 0.2;
        r->type = RT_RECT;
        r->texture = &player_tex;

        Collidable *c = checs_component_get(7, player);
        c->r = 0.1f;
        c->mass = 10.f;
        c->behaviourType = DYNAMIC;
        c->type = CIRCLE;

        Animation *a = checs_component_get(4, player);
        a->pattern = &player_animationPattern;
        a->frameDelay = 20;


        EntityId camera = checs_entity(1 << 1 | 1 << 2 | 1 << 6);
        render_rectBatch_camera_set(camera);
        
        Camera *camera_c = checs_component_get(6, camera);
    ...
    {
        EntityId floor = checs_entity(1 << 1 | 1 << 5 | 1 << 7);
        Transform *t = checs_component_get(1, floor);
        t->pos[0] = 0.f;
        t->pos[1] = -2.f;

        Renderable *r = checs_component_get(5, floor);
        r->scale[0] = 4.f;
        r->scale[1] = 0.5f;
        r->texture = &cobble;
        r->type = RT_RECT;

        Collidable *c = checs_component_get(7, floor);
        c->bb[0] = 2.f;
        c->bb[1] = 0.25f;
        c->mass = 0.5f;
    ...
    EntityId wall_left;
    {
        wall_left = checs_entity(1 << 1 | 1 << 5 | 1 << 7);
        Transform *t = checs_component_get(1, wall_left);
        t->pos[0] = 2.f;
        t->pos[1] = 0.f;

        Renderable *r = checs_component_get(5, wall_left);
        r->scale[0] = 0.5f;
        r->scale[1] = 4.f;
        r->texture = &cobble;
        r->type = RT_RECT;

        Collidable *c = checs_component_get(7, wall_left);
        c->bb[0] = 0.25f;
        c->bb[1] = 2;
        c->mass = 0.5f;
        c->behaviourType = STATIC;
    ...
    Transform *t = checs_component_get(1, wall_right);
    t->pos[0] = -2;
...
void game_state_update(State *s)
{
	...
	//printf("%f ms / frame, Entitys: %u, ms / entity %f\n", checs_ms_per_frame_get(), checs_component_count(1), checs_ms_per_frame_get() / checs_component_count(1));

	EntityId player = checs_entity_get_by_name("player");
    Velocity *const v = checs_component_get(2, player);

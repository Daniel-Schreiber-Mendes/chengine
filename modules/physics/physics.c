...
{
	checs_component_register(<Collidable>, sizeof(Collidable), NULL, NULL, 1 << <Transform>);
	checs_tag_register(<T:Gravitatable>);
	checs_command_register(<C:CollisionCommand>);
...
{
	EntityId e;
	for (uint32_t i=0; checs_components_iterate(<Collidable>, &e, i); ++i)
	{
		//add entity to all neighbouring partitions if it overlaps
		Transform const *const t = checs_component_get(<Transform>, e); //TODO: solve bug when accessing component
		Collidable const *const c = checs_component_get(<Collidable>, e);
		int16_t x0, x1, y0, y1;
		if (c->type == CIRCLE)
		{
	...
	for (uintEC xi=0; xi < PARTITION_COUNT; ++xi)
	{
		for (uintEC yi=0; yi < PARTITION_COUNT; ++yi)
		{
			...
			for (uintEC i=0; i < p->size; ++i)
			{
				...
				Collidable const *const c0 = checs_component_get(<Collidable>, collider);
				Transform *const t0 = checs_component_get(<Transform>, collider);
				...
				for (uintEC j = 0 + i + 1; j < p->size; ++j)
				{
					...
					Collidable const *const c1 = checs_component_get(<Collidable>, collidable);
					Transform const *const t1 = checs_component_get(<Transform>, collidable);

					if (collision_tests[c0->type][c1->type](t0, t1, c0, c1, &contact))
					{
						Velocity const *const v0 = checs_component_get(<Velocity>, collider);
						Velocity const *const v1 = checs_component_get(<Velocity>, collidable);
				...
				for (uintEC j = 0; j < p->static_size; ++j)
				{
					...
					if (collision_tests[c0->type][c1->type](t0, t1, c0, c1, &contact))
					{
						Velocity *const v0 = checs_component_get(<Velocity>, collider);
...
void physics_gravity_task(void)
{
	...
	Velocity *v0;
	EntityId e;
	for (uintEC i=0; e = checs_tags_iterate(<T:Gravitatable>, i); ++i)
	{
		v0 = checs_component_get(<Velocity>, e);

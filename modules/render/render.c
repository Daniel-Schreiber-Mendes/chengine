...
{
	checs_component_register(<Renderable>, sizeof(Renderable), NULL, NULL, 1 << <Transform>);
...
static void rectBatch_render(void)
{
	...
	if (checs_component_count(<Renderable>) > rectBatch.renderableCount)
	{
		glBufferData(GL_ARRAY_BUFFER, checs_component_count(<Renderable>) * rectBatch.vao.instance_size, NULL, GL_STREAM_DRAW); //update instance buffer size
		rectBatch.renderableCount = checs_component_count(<Renderable>);
	}
	...
	Renderable *r;
	uintEC i = 0;
	for (EntityId entity; r = checs_components_foreach(<Renderable>, &entity, i); ++i)
	{
    	assert(r->texture);
    	...
    	glm_translate(transform, checs_component_get(<Transform>, entity));

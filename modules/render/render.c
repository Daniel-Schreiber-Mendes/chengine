...
{
	checs_component_register(<Renderable>, Renderable, 200, NULL, NULL);
	checs_tasks_register(CHECS_ON_DRAW, render_task);
}
...
void render_task(void)
{
	...
	rectBatch.elementCount = 0;
		uint32_t textureLayers[checs_component_count(<Renderable>)];
		vao_bind(&rectBatch.vao);

		bo_update(Vbo, rectBatch.pi_vbo, float, p, ({
			Renderable *r;
			Transform *t;
			checs_components_foreach(<Renderable>, r, entity, ({
				t = checs_component_get(<Transform>, entity);
		...
		program_uniform1uv_set(rectBatch.program, "u_texture_layers", checs_component_count(<Renderable>), textureLayers);

		textureManager_texture_bind();
		glDrawElementsInstanced(rectBatch.mode, rectBatch.elementCount, GL_UNSIGNED_INT, NULL, checs_component_count(<Renderable>));

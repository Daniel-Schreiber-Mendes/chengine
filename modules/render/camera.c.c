...
{
    checs_component_register(6, sizeof(Camera), NULL, NULL, 1 << 1);
}
...
void camera_vp_recalculate(EntityId const id, mat4 vp)
{
    ...
    Transform *t = checs_component_get(1, id);
    Camera *c = checs_component_get(6, id);

    if (c->target)
    {
        Transform *target_t = checs_component_get(1, c->target);

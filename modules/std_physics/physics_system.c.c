...
void physics_task(void)
{
	...
	checs_components_foreach(6, Collidable, c0, collider, ({
		checs_component_get(6, Collidable, c0, collider);

		if (c0->behaviourType != DYNAMIC) continue;

		checs_component_get(5, Velocity, v0, collider);

		//if (!v0->vel[0] && !v0->vel[1]) continue;
		{
			checs_components_foreach(6, Collidable, c1, collidable, ({
				if (collidable == collider) continue;
				checs_component_get(1, Transform, t0, collider);
				checs_component_get(1, Transform, t1, collidable);


				if (collision_tests[c0->type][c1->type](t0, t1, c0, c1))
				{
					checs_command_publish(CollisionCommand, 6, (&(CollisionCommand){collider, collidable}));
					vec2 impulse;
					float contactv;
					...
					{
						checs_component_get(5, Velocity, v1, collidable)
...
{
	checs_component_mut_use(Velocity, v0);
	checs_attribute_entity_foreach(0, gravitatable)
	{
		checs_component_get(5, Velocity, v0, gravitatable);

...
void physics_task(void)
{
	...
	checs_components_foreach(<Collidable>, Collidable, c0, collider, ({
		checs_component_get(<Collidable>, Collidable, c0, collider);

		if (c0->behaviourType != DYNAMIC) continue;

		checs_component_get(<Velocity>, Velocity, v0, collider);

		//if (!v0->vel[0] && !v0->vel[1]) continue;
		{
			checs_components_foreach(<Collidable>, Collidable, c1, collidable, ({
				if (collidable == collider) continue;
				checs_component_get(<Transform>, Transform, t0, collider);
				checs_component_get(<Transform>, Transform, t1, collidable);
				...
				if (collision_tests[c0->type][c1->type](t0, t1, c0, c1))
				{
					...
					{
						checs_component_get(<Velocity>, Velocity, v1, collidable)
...
void physics_gravity_task(void)
{
	...
	{
		checs_component_get(<Velocity>, Velocity, v0, gravitatable);

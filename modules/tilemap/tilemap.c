...
{
	checs_component_register(<Tile>, sizeof(Tile), NULL, NULL, 1 << <Transform> | 1 << <Renderable>);
}
...
void chunk_generate(uint8_t cx, uint8_t cy)
{
	...
	for (uint8_t tx=0; tx < CHUNK_SIZE; ++tx)
	{
		for (uint8_t ty=0; ty < CHUNK_SIZE; ++ty)
		{
			...
			if (n == 3)
			{
				e = checs_entity(1 << <Renderable> | 1 << <Tile> | 1 << <Transform> | 1 << <Collidable>);
			...
			{
				e = checs_entity(1 << <Renderable> | 1 << <Tile> | 1 << <Transform>);
			}
